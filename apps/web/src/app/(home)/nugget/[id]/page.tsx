...
import { getDailyIdeas, getIdeaById } from "@/lib/server-api";
...
export async function generateMetadata({
	params,
}: NuggetPageProps): Promise<Metadata> {
	...
	try {
		...
		if (!idea) {
			return {
				...
				description:
					"Explore AI startup ideas with market research and practical next steps.",
			};
		}
		...
		const rawDesc =
			...
			"Explore validated AI startup ideas with clear insights and practical steps.";
		const description =
...
export default async function NuggetDetailPage({ params }: NuggetPageProps) {
	...
	try {
		...
		console.log("[DEBUG] Page: User authentication status:", {
			isAuthenticated,
		});
		...
		if (isAuthenticated) {
			...
			if (!idea) {
				...
				console.log(
					"[DEBUG] Page: No server data for nugget, using client fallback",
				);
				return (
			...
			console.log(
				"[DEBUG] Page: Server data fetch successful, rendering IdeaDetailsView",
			);
			...
			const articleSchema = {
				...
				description:
					ideaWithFullData.executiveSummary || ideaWithFullData.description,
				...
				datePublished: ideaWithFullData.createdAt
					? new Date(ideaWithFullData.createdAt).toISOString()
					: undefined,
				dateModified: ideaWithFullData.updatedAt
					? new Date(ideaWithFullData.updatedAt).toISOString()
					: undefined,
			};
			...
			const productSchema = {
				...
				aggregateRating: {
					...
					ratingValue: ideaWithFullData.ideaScore?.totalScore
						? (ideaWithFullData.ideaScore.totalScore / 10).toFixed(1)
						: "4.5",
		...
		console.log(
			"[DEBUG] Page: Non-authenticated user, using client-side tracking",
		);
		return (
			<div className="min-h-screen bg-background">
		...
		if (
			...
			console.log(
				"[DEBUG] Page: Redirecting to pricing due to view limit exceeded",
			);
			redirect("/pricing?reason=view_limit");
		}
		...
		console.log(
			"[DEBUG] Page: Using client fallback for error:",
			error?.message,
		);
		return (
			<div className="min-h-screen bg-background">
