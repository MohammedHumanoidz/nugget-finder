  ...
  if (error?.message) {
    ...
    if (message.includes('payment') || message.includes('card')) {
      return {
        code: 'PAYMENT_ERROR',
        message: error.message,
        userMessage: 'There was an issue with your payment method. Please check your payment details.',
        retryable: true,
      };
    }
    ...
    if (message.includes('subscription') && message.includes('not found')) {
      return {
        ...
        message: error.message,
        userMessage: 'No active subscription found. Please contact support if this seems incorrect.',
        retryable: false,
      };
    }
    ...
    if (message.includes('plan') || message.includes('price')) {
      return {
        code: 'INVALID_PLAN',
        message: error.message,
        userMessage: 'The selected plan is no longer available. Please refresh the page and try again.',
        retryable: true,
      };
    }
...
export function shouldRetryError(error: SubscriptionError, attemptCount: number): boolean {
