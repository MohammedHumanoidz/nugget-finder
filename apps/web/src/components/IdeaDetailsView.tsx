...
const IdeaDetailsView: React.FC<IdeaDetailsViewProps> = ({ idea }) => {
    ...
    return (
      <ul className="space-y-1 pl-4">
        {items.map((item, index) => (
          <li key={index.toString()} className="text-muted-foreground text-sm">
            • {item}
          </li>
        ))}
      </ul>
  ...
  const financialData = idea.monetizationStrategy.financialProjections.map(
    (proj) => ({
      year: `Year ${proj.year}`,
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="max-w-7xl mx-auto px-4 pb-8 text-center">
        <div className="flex items-end justify-center mb-10">
          <Image src="/checkout-this-nugget.png" alt="Check this nugget" width={100} height={100}/>
        <h1 className="font-bold text-primary mb-6 ml-2 font-mono text-3xl">
          CHECK OUT THIS NUGGET ❤️‍🔥
        </h1>
        </div>
          ...
          <div className="flex gap-2 mt-4 items-center justify-center">
            <div className="flex gap-2 flex-wrap">
              {idea.tags.map((tag, index) => (
                <span
                  key={index.toString()}
                  className="bg-muted text-primary border border-border px-3 py-1 rounded-full text-sm font-medium"
                >
                  {tag}
                </span>
              ))}
              ...
              <div className="p-6 space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  {idea.marketCompetition.competitorFailurePoints &&
                    idea.marketCompetition.competitorFailurePoints.length >
                      0 && (
                      <div>
                        <h4 className="text-foreground font-semibold mb-2">
              ...
              <div className="p-6 space-y-6">
                ...
                <div className="flex gap-8 text-sm">
                  ...
                  <span>
                    ...
                    {idea.monetizationStrategy.confidence}/10
                  </span>
                </div>
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-3 gap-4 mb-4">
                      <div>
                        • <strong>LTV:</strong>{" "}
                        {formatCurrency(
                          idea.monetizationStrategy.keyMetrics.ltv
                        )}
                      </div>
                      <div>
                        • <strong>CAC:</strong>{" "}
                        {formatCurrency(
                          idea.monetizationStrategy.keyMetrics.cac
                        )}
                      </div>
                      <div>
            ...
            <div className="bg-card border border-border rounded-lg shadow-sm overflow-hidden">
              ...
              <div className="p-4 space-y-4">
                <div className="text-center bg-muted/50 border border-border text-primary p-3 rounded-lg">
                  <strong>
                    Overall Idea Score: {idea.ideaScore.totalScore}/100
                  </strong>
                </div>
