...
const NuggetCard = ({ nugget, className = "" }: CardProps) => (
  <Card
    key={nugget.id}
    className={`rounded-xl border border-border bg-card shadow-sm transition-transform duration-300 hover:-translate-y-1 hover:shadow-lg h-fit w-sm ${className}`}
  >
    <CardHeader className="pb-2">
      <CardTitle className="text-xl font-semibold">{nugget.title}</CardTitle>
      {nugget.narrativeHook && (
        <p className="mt-2 italic text-muted-foreground text-primary/80">
          {nugget.narrativeHook}
        </p>
      )}
    </CardHeader>
    <CardContent className="space-y-3">
      {nugget.problemStatement && (
        <div>
          <p className="text-sm text-foreground">{nugget.problemStatement}</p>
        </div>
      )}
      {nugget.description && (
        <p className="text-sm text-muted-foreground line-clamp-3">{nugget.description}</p>
      )}
  ...
  return (
    <section className="w-full">
      <div className="mx-auto max-w-7xl px-4">
        ...
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-[1fr_1.5fr_1fr] items-center justify-center">
          {/* Left Column */}
          ...
          <div className="flex flex-col items-center justify-center">
            <NuggetCard
              nugget={centerCard}
              className="scale-105 shadow-lg border-primary/50"
            />
          </div>
