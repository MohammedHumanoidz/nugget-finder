...
interface BrowseServerClientProps {
  initialIdeas: Idea[];
  initialLimits: UserIdeaLimits | null;
}

export default function BrowseServerClient({ initialIdeas, initialLimits }: BrowseServerClientProps) {
  ...
  // Get user limits - use initial data and refresh in background
  const { data: limits, refetch: refetchLimits } = useQuery({
    ...trpc.ideas.getLimits.queryOptions(),
    initialData: initialLimits || undefined,
  ...
  // Infinite query for browsing ideas - use initial data
  const {
    data,
    fetchNextPage,
    hasNextPage,
    ...
    initialData: initialIdeas.length > 0 ? {
      pages: [initialIdeas],
      pageParams: [0],
    } : undefined,
    staleTime: 30000, // Consider data fresh for 30 seconds
