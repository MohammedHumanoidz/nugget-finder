...
export const dailyIdeaGenerationJob = schedules.task({
  ...
  run: async (payload) => {
    ...
    let successCount = 0;
    let errorCount = 0;
    const results: Array<{ attempt: number; success: boolean; ideaId?: string; error?: string }> = [];
    ...
    for (let i = 1; i <= 4; i++) {
      try {
        ...
        if (ideaId) {
          ...
          errorCount++;
          results.push({ attempt: i, success: false, error: "generateDailyIdea returned null" });
          console.error(`❌ Idea ${i} generation failed: No idea ID returned`);
          
          debugLogger.logError("Daily idea generation failed", new Error("generateDailyIdea returned null"), {
            attempt: i
          });
        }
      } catch (error) {
    ...
    if (successCount > 0) {
      ...
      debugLogger.logError("Daily idea generation job failed completely", new Error("All idea generation attempts failed"), {
        ...summary
      });
    }
