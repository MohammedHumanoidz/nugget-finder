...
export const onDemandIdeaGenerationJob = task({
  ...
  run: async (payload: { requestId: string; userPrompt: string; userId: string }) => {
    const { requestId, userPrompt, userId } = payload;
    
    try {
      console.log(`🚀 Starting On-Demand Idea Generation for request: ${requestId}`);
        ...
        try {
          ...
          await prisma.ideaGenerationRequest.update({
            ...
            data: {
              ...
              progressMessage: "Setting research direction based on your prompt...",
              imageState: "confused"
            }
          });
          
          const researchDirection = await IdeaGenerationAgentController.masterResearchDirector(agentContext);
          ...
          const trends = await IdeaGenerationAgentController.trendResearchAgent(agentContext, researchDirection || undefined);
          if (!trends) {
          ...
          const problemGaps = await IdeaGenerationAgentController.problemGapAgent(agentContext);
          if (!problemGaps) {
          ...
          const competitive = await IdeaGenerationAgentController.competitiveIntelligenceAgent(agentContext);
          if (!competitive) {
          ...
          const monetization = await IdeaGenerationAgentController.monetizationAgent(agentContext);
          if (!monetization) {
          ...
          const whatToBuild = await IdeaGenerationAgentController.whatToBuildAgent(agentContext);
          ...
          const initialIdea = await IdeaGenerationAgentController.ideaSynthesisAgent(agentContext);
          if (!initialIdea) {
          ...
          const refinementPrompt = await IdeaGenerationAgentController.criticAgent([initialIdea], agentContext);
          ...
          const finalIdea = await IdeaGenerationAgentController.ideaSynthesisAgent(agentContext, refinementPrompt || undefined);
          if (!finalIdea) {
      ...
      if (generatedIdeas.length > 0) {
        ...
        debugLogger.info("🎉 On-demand idea generation background job completed", {
          requestId,
          userPrompt,
          userId,
      ...
      console.error("❌ On-Demand Idea Generation Background Job Failed:", error);
      ...
      debugLogger.logError("On-Demand Idea Generation Background Job", error as Error, {
        requestId,
        userPrompt,
        userId,
      });
