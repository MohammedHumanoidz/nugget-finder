 ...
 */
export async function getUserIdeaLimits(
	userId: string,
): Promise<UserIdeaLimits> {
	try {
		...
		const claimsRemaining =
			user.claimLimit === -1
				? -1
				: Math.max(0, user.claimLimit - user.claimsUsed);
		const savesRemaining =
			user.saveLimit === -1 ? -1 : Math.max(0, user.saveLimit - user.savesUsed);
		const viewsRemaining =
			user.viewLimit === -1 ? -1 : Math.max(0, user.viewLimit - viewsUsed);
 ...
 */
export async function unsaveIdeaForUser(
	userId: string,
	ideaId: string,
): Promise<{ success: boolean; message: string }> {
	try {
 ...
 */
export async function claimIdeaForUser(
	userId: string,
	ideaId: string,
): Promise<{ success: boolean; message: string }> {
	try {
 ...
 */
export async function unclaimIdeaForUser(
	userId: string,
	ideaId: string,
): Promise<{ success: boolean; message: string }> {
	try {
 ...
 */
export async function recordIdeaView(
	userId: string,
	ideaId: string,
): Promise<void> {
	try {
...
export async function getIdeasForUser(
	...
	try {
		...
		return ideas.map((idea) => ({
			...
			isClaimedByOther:
				idea.claimedIdeas && idea.claimedIdeas.userId !== userId,
		}));
	} catch (error) {
