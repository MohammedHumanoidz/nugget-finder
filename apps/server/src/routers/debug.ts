...
export const debugRouter = router({
	...
	checkPlans: publicProcedure.query(async () => {
		try {
			...
			console.log(
				"📦 Stripe plans:",
				stripePlans.map((p) => ({ id: p.id, name: p.name })),
			);

			// Get plans from Better Auth
			const authContext = auth;
			const betterAuthPlans = await (authContext as any).options.plugins
				.find((p: any) => p.id === "stripe")
				?.subscription?.plans?.();

			console.log(
				"🔐 Better Auth plans:",
				betterAuthPlans?.map((p: any) => ({ id: p.id, name: p.name })),
			);

			return {
				stripePlans: stripePlans.map((p) => ({
					id: p.id,
					name: p.name,
					price: p.price,
				})),
				betterAuthPlans:
					betterAuthPlans?.map((p: any) => ({
						id: p.id,
						name: p.name,
						price: p.price,
					})) || [],
