  ...
  suggestions: publicProcedure
    ...
    .query(async ({ input }) => {
      try {
        ...
        const suggestions = generateSearchSuggestions(ideas, partialQuery, limit);
  ...
  analytics: publicProcedure
    .query(async () => {
      try {
        ...
        const popularTerms = [
          "AI", "SaaS", "mobile app", "e-commerce", "productivity", 
          "healthcare", "education", "fintech", "automation", "analytics"
        ];
...
async function loadSearchableIdeas(): Promise<SearchableIdea[]> {
  ...
  return ideas.map(idea => {
    ...
    searchableIdea.searchCorpus = TextProcessor.createSearchCorpus(searchableIdea);
...
function getPersonalizationMatches(
  ...
  if (personalization.skills) {
    ...
    const matchedSkills = skills.filter(skill => 
      skill.length > 2 && ideaText.includes(skill)
    );
  ...
  if (personalization.categories) {
    ...
    const matchedCategories = categories.filter(cat => 
      cat.length > 2 && ideaText.includes(cat)
    );
