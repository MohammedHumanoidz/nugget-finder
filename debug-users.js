...
async function checkUsers() {
  ...
  try {
    ...
    users.forEach(user => {
      console.log(`- ${user.email}: viewLimit=${user.viewLimit}, viewsUsed=${user.viewsUsed}, lastViewReset=${user.lastViewReset}`);
    });
    ...
    viewedIdeas.forEach(view => {
      console.log(`- ${view.user.email} viewed ${view.ideaId} at ${view.createdAt}`);
    });
